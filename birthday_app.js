...
class BirthdayApp {
   ...
   /* adjustWordsPosition() {
        ...
        if (words && garden) {
            // Checks if we are on mobile (screen width less than 768px)
            if (window.innerWidth < 768) {
                // MOBILE SETTINGS
                
                // 1. Force the text container to align with the screen top
                words.style.top = "0px"; 
                words.style.left = "0px";
                
                // 2. Move the text down to the bottom half of the screen
                // Increase '350' to move text LOWER. Decrease to move HIGHER.
                const mobileOffset = 350; 
                
                words.style.marginTop = mobileOffset + "px";
                
            } else {
                // DESKTOP SETTINGS (Keep original logic)
    ...
    if (words && garden) {
        ...
        // HORIZONTAL: Align exactly with the garden, no extra +70
        words.style.left = garden.offsetLeft + "px";
        words.style.width = garden.offsetWidth + "px"; // Force container to match cake width
        
        // VERTICAL: 
        // 255 is overlapping the middle layer. 
        // Try 180 to put it in the upper gap, or 320 for the lower gap.
        words.style.top = (garden.offsetTop + 230) + "px"; 
    }
}
